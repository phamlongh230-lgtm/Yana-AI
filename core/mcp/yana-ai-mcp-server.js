#!/usr/bin/env node
/**
 * Yana AI MCP Server — exposes facts, gates, and hooks as MCP tools.
 * Any MCP-compatible agent (Claude, Cursor, etc.) can call these directly.
 *
 * Protocol: Model Context Protocol (MCP) stdio transport
 * Start: node core/mcp/yana-ai-mcp-server.js
 * Or via Claude Code settings.json mcpServers entry.
 *
 * Exposed tools:
 *   yana-ai_facts_search    — search L1 atomic facts by keyword
 *   yana-ai_facts_add       — add a new L1 fact
 *   yana-ai_gate_check      — check if an action passes the gate stack
 *   yana-ai_risk_score      — get risk score for a proposed action
 *   yana-ai_session_status  — current session state (trust, circuit, budget)
 *   yana-ai_checkpoint      — create a session checkpoint
 */
...
function factsAdd({ id, type = 'fact', statement, confidence = 'unverified', scope = 'Yana AI', tags = [] }) {
...
source: mcp:yana-ai-mcp-server:${now}
...
function gateCheck({ command, tool = 'Bash' }) {
  ...
  const result = spawnSync('bash', [safeRunPath, '--engine', 'claude', ...command.split(' ')], {
    ...
    env: { ...process.env, YANA_SAFE_RUN_DRY_RUN: '1' }
  });
...
const TOOLS = [
  { name: 'yana-ai_facts_search', description: 'Search L1 atomic memory facts by keyword and/or tag.',
    inputSchema: { type: 'object', properties: {
      keyword: { type: 'string', description: 'Keyword to search in fact content' },
      tag: { type: 'string', description: 'Tag to filter by' }
    }}},
  { name: 'yana-ai_facts_add', description: 'Add a new L1 atomic fact to Yana AI memory.',
  ...
  { name: 'yana-ai_gate_check', description: 'Check if a command passes the Yana AI safe-run gate.',
    inputSchema: { type: 'object', required: ['command'], properties: {
      command: { type: 'string', description: 'Shell command to validate' }
    }}},
  { name: 'yana-ai_session_status', description: 'Get current Yana AI session status: circuit breaker, token budget, risk log, active agents.',
    inputSchema: { type: 'object', properties: {} }},
  { name: 'yana-ai_checkpoint', description: 'Create a session checkpoint (git state + L2 facts snapshot).',
    inputSchema: { type: 'object', properties: {
...
rl.on('line', (line) => {
  ...
  if (method === 'initialize') {
    send({ jsonrpc: '2.0', id, result: {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: 'yana-ai', version: '1.6.0' }
    }});
    ...
    try {
      switch (name) {
        case 'yana-ai_facts_search':  result = factsSearch(args); break;
        case 'yana-ai_facts_add':     result = factsAdd(args); break;
        case 'yana-ai_gate_check':    result = gateCheck(args); break;
        case 'yana-ai_session_status': result = sessionStatus(); break;
        case 'yana-ai_checkpoint':    result = sessionCheckpoint(args); break;
