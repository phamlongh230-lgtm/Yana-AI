...
const client = new Etcd3({
  ...
  auth:  { username: 'yana-ai', password: process.env.ETCD_PASSWORD! },
})

// Put
await client.put('/yana-ai/config/model-tier').value('power')

// Get
const tier = await client.get('/yana-ai/config/model-tier').string()
console.log('[etcd] tier:', tier)

// Delete
await client.delete().key('/yana-ai/config/deprecated').exec()

// Get all under prefix
const entries = await client.getAll().prefix('/yana-ai/config/').strings()
...
await lease.put('/yana-ai/agents/agent-1').value(JSON.stringify({
  id:      'agent-1',
...
lease.on('lost', async (err) => {
  ...
  // Re-register
  await lease.put('/yana-ai/agents/agent-1').value(...)
})
```
...
await withLock('/yana-ai/locks/schema-migration', async () => {
