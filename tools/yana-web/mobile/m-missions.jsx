        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 10, width: "100%" }}>
          <div style={{ lineHeight: 1.3, minWidth: 0, flex: 1 }}>
            ...
            <div style={{ fontSize: 11.5, color: "var(--ink-3)", marginTop: 2 }}>
              {m.owner} · {L("due", "hạn", "마감", "截止")} {m.due} · <span style={{ color: statusColor }}>{m.status}</span>
            </div>
          </div>
...
function MMissions() {
  ...
  const filters = [
    ["active", L("Active", "Đang chạy", "진행 중", "进行中")],
    ["all", L("All", "Tất cả", "전체", "全部")],
    ["recurring", L("Recurring", "Định kỳ", "반복", "周期性")],
  ];
  ...
  return (
    <div data-screen-label="Mission Center" style={{ display: "flex", flexDirection: "column", gap: "var(--gap)" }}>
      <MHead title={L("Missions", "Nhiệm vụ", "미션", "任务")} sub={L("Multi-agent work, visible end to end.", "Công việc đa tác nhân, theo dõi đầu đến cuối.", "멀티 에이전트 작업을 처음부터 끝까지 확인.", "多智能体协作，全程可见。")} />
