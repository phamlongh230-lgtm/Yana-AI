...
// ── Skill index (loaded once at startup) ─────────────────────────────────────
let SKILL_INDEX = [];
const INDEX_PATH = path.resolve(__dirname, '..', '..', 'core', 'config', 'skill-trigger-index.json');
try {
  SKILL_INDEX = JSON.parse(fs.readFileSync(INDEX_PATH, 'utf8'));
} catch (e) {
  // Index not built yet — skill matching disabled, falls back to signal-based
}

// ── Irreversible/external signals (highest precedence) ───────────────────────
...
/**
 * matchSkills(task) — find best matching skills from the index.
 * Returns top-3 matches sorted by score (number of trigger hits).
 */
  ...
  for (const entry of SKILL_INDEX) {
    ...
    for (const trigger of entry.triggers) {
      if (lower.includes(trigger)) {
        ...
        // Try reversed word order for 2-word triggers (e.g. "code review" ↔ "review code")
        const parts = trigger.split(' ');
        if (parts.length === 2) {
          const rev = `${parts[1]} ${parts[0]}`;
          if (lower.includes(rev)) {
            score += 1; // No length bonus — less confident match
            hits.push(trigger);
          }
        }
      }
...
/**
 * classify(task) → decision object
 *
 * Priority:
 *   1. External signals (irreversible) — always block for human confirm
 *   2. Skill index match — if high-confidence, return matched skill(s)
 *   3. Complex signals — multi-step work
 *   4. Default: simple
 */
function classify(task) {
  if (typeof task !== 'string') task = String(task || '');

  // 1. External gate — irreversible actions always need confirm
...
module.exports = { classify, matchSkills };
